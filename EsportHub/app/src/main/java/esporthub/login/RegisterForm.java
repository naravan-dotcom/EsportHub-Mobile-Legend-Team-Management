...
public class RegisterForm extends VBox {
    ...
    public RegisterForm(Runnable onLoginLinkClick, BiConsumer<String, String> onRegisterSubmit) {
        // Form layout rules
        ...
        Label subtitle = new Label("Daftar untuk bergabung dengan komunitas");
        ...
        setupButtonStyling(registerBtn);

        registerBtn.setOnAction(e -> {
            alertContainer.getChildren().clear();
            String u = usernameField.getText().trim();
            String p = passwordField.getText();
            onRegisterSubmit.accept(u, p);
        });

        // Switch Link back to Login View
        HBox toggleContainer = new HBox(5);
        toggleContainer.setAlignment(Pos.CENTER);

        Label hasAccountLabel = new Label("Sudah punya akun?");
        hasAccountLabel.setTextFill(Color.web("#8c92a6"));
        hasAccountLabel.setFont(Font.font("Inter", 12.5));

        Hyperlink loginLink = new Hyperlink("Masuk Sekarang");
        loginLink.setTextFill(Color.web("#7c4ded"));
        loginLink.setFont(Font.font("Inter", FontWeight.BOLD, 12.5));
        loginLink.setUnderline(true);
        loginLink.setStyle("-fx-border-color: transparent; -fx-padding: 0;");
        loginLink.setOnAction(e -> {
            alertContainer.getChildren().clear();
            onLoginLinkClick.run();
        });

        toggleContainer.getChildren().addAll(hasAccountLabel, loginLink);

        this.getChildren().addAll(header, fields, registerBtn, toggleContainer);
    }
